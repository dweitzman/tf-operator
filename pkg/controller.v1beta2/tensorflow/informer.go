...
import (
	...
	tfjobinformersv1beta2 "github.com/kubeflow/tf-operator/pkg/client/informers/externalversions/kubeflow/v1beta2"
	"github.com/kubeflow/tf-operator/pkg/common/util/v1beta2/unstructured"
...
func NewUnstructuredTFJobInformer(restConfig *restclientset.Config, namespace string) tfjobinformersv1beta2.TFJobInformer {
	dynClientPool := dynamic.NewDynamicClientPool(restConfig)
	dclient, err := dynClientPool.ClientForGroupVersionKind(tfv1beta2.SchemeGroupVersionKind)
	if err != nil {
		panic(err)
	}
	resource := &metav1.APIResource{
		Name:         tfv1beta2.Plural,
		SingularName: tfv1beta2.Singular,
		Namespaced:   true,
		Group:        tfv1beta2.GroupName,
		Version:      tfv1beta2.GroupVersion,
	}
...
func (tc *TFController) NewTFJobInformer(tfJobInformerFactory tfjobinformers.SharedInformerFactory) tfjobinformersv1beta2.TFJobInformer {
	return tfJobInformerFactory.Kubeflow().V1beta2().TFJobs()
}

func (tc *TFController) getTFJobFromName(namespace, name string) (*tfv1beta2.TFJob, error) {
	key := fmt.Sprintf("%s/%s", namespace, name)
	return tc.getTFJobFromKey(key)
}

func (tc *TFController) getTFJobFromKey(key string) (*tfv1beta2.TFJob, error) {
...
func tfJobFromUnstructured(obj interface{}) (*tfv1beta2.TFJob, error) {
	...
	var tfjob tfv1beta2.TFJob
	err := runtime.DefaultUnstructuredConverter.FromUnstructured(un.Object, &tfjob)
	logger := tflogger.LoggerForUnstructured(un, tfv1beta2.Kind)
	if err != nil {
	...
	err = validation.ValidateBetaTwoTFJobSpec(&tfjob.Spec)
	if err != nil {
		logger.Errorf(failedMarshalMsg, err)
		return nil, errFailedMarshal
	}
...
func unstructuredFromTFJob(obj interface{}, tfJob *tfv1beta2.TFJob) error {
	un, ok := obj.(*metav1unstructured.Unstructured)
